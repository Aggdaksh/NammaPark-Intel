"""PostgreSQL/PostGIS schema setup for NammaPark Intel."""
...
def run_schema(database_url: str) -> None:
    try:
        ...
        raise RuntimeError("psycopg is required to apply the PostgreSQL schema; run `uv sync` first") from exc
...
def main() -> None:
    ...
    parser.add_argument("--write-sql", default=None)
    args = parser.parse_args()
    if args.write_sql:
        print(write_schema(args.write_sql))
    elif args.database_url:
        run_schema(args.database_url)
